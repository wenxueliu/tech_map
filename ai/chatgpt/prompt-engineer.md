...
第二步：我提供你代码片段，你将分析每个方法，根据Hibernate Validator 注解解析参数，以json的格式生成参数规则
第三步：给出3条关于参数规则的优化建议，不断询问我是否需要删除或增加哪些参数规则，直到我回复“确认”。以json的格式输出确认的参数规则，继续下一步。
第四步：针对参数规则，以json的格式生成测试用例数据。
第五步：给出3条关于参数规则的优化建议，不断询问我删除或增加哪些用例数据，直到我回复“确认”。我确认后以 json 格式输出确认的用例数据，继续下一步，
第六步：根据每个用例数据依次生成对应的用例代码。

要求
1、采用2-wise方法生成测试用例数据。
...
7、每个测试用例代码包括数据准备，mock依赖，mock行为，断言。
User:"""代码片段"""
```
